...
iprint:
  ...
  push	edx
  ; preserve edx on the stack to be restored after the function runs
  push	esi
...
divideLoop:
  ...
  ; divide eax by esi
  add	edx, 48	
  ; convert edx to it's ascii representation - edx holds the remainder 
  ; after a division
  push	edx
  ; push edx (string represenation of an integer) onto the stack
  cmp	eax, 0
  ; can the integer be divided anymore?
  jnz	divideLoop
  ; jump if not zero to be the label divideLoop

printLoop:
  dec	ecx
  ; count down each byte that we put on the stack
  mov	eax, esp
  ; move the stack pointer into esp for printing
  call	sprint
  ; call our string print function
  pop	eax
  ; remove last character from the stack to move esp forward
  cmp	ecx, 0
  ; have we printed all the bytes we pushed onto the stack
  jnz	printLoop
  ; jump is not zero to the last print label
...
iprintLF:
  ...
  pop	eax
  ; restor the original value of eax before our function was called
  ret
...
sprintLF:
  ...
  push	eax
  ; push eax onto the stack to preserve it while we use the eax register in 
  ; this function
  mov	eax, 0Ah
  ; move 0Ah into eax - 0Ah is the ascii character for a linefeed
  push	eax
  ...
  ret
  ; return to our regularly schedule program already in progress
