...
; we can modify this now without having to update anywhere else
...
_start:
  ...
  mov 	eax, ebx
  ; move the address in ebx into eax as well
...
nextchar:
  ...
  jz	finished
  ; jump (if the zero flagged has been set) to the point in the code labeled 
  ; 'finished'
  inc	eax
  ; increment the address in EAX by one byte (if the zero flagged has NOT
  ; been set)
  jmp	nextchar
...
finished:
  ...
  ; but EAX has been incremented one byte for each character in the message
  ; string when you subtract one memory address from another of the same type
  ; the result is the number of segments between them - in this case the 
  ; number of bytes
  mov	edx, eax
